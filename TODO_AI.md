...
## Done
- [x] 2026-09-16 · Rebuilt the header as a two-cell flex layout (text cell + photo cell) instead of a background-image wash — fixes the text/photo overlap problem structurally rather than by tuning opacity/scale percentages (two earlier attempts at that both turned out to be band-aids). Photo now shows at full clarity, `cover`-fit with rounded corners in its own box; stacks full-width-on-top on mobile.
