...
## Done
- [x] 2026-09-16 · Reworked default light theme to match Sillan's own posters (owner-supplied images as the target look): chips are plain black/white by default. Added a "Colour-blind" slide toggle next to the theme switch (off by default) that restores the previous Okabe-Ito colour+shape coding. Updated CLAUDE.md's hard constraint wording accordingly.
- [x] 2026-09-16 · Legend is now collapsed by default behind a "Legend" toggle (same `<details>`/`<summary>` pattern as the announcements bar).
- [x] 2026-09-16 · UI polish pass: moved "M3 = does not call at this stop" from the footer to under the To Dublin table; default theme is now always light on load (no longer follows device `prefers-color-scheme`); centred the "Stop" column header and removed the To Dublin subtitle; legend now wraps to a second line after Friday, with the redundant divider line beneath it removed; run headers show a plain number instead of "Run N".
