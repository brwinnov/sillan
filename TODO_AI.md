...
## Done
- [x] 2026-09-16 · Header background photo settled (after several same-day iterations — see `CONTEXT_MAP.md`'s "Header" section for the full history): a `background-image` with a semi-transparent red gradient over the bus photo, `auto 100%` height (no cropping), centred. A two-cell flex layout was tried in between but reverted — it fixed text overlap but then the photo (as its own box) competed visually with the header buttons/WhatsApp badge instead.
- [x] 2026-09-16 · Extended the poster-style board card to "From Dublin" and both Saturday & Sunday tables, matching "To Dublin". Removed From Dublin's "8 departures a day..." subtitle; moved the Sat/Sun "weekend return trips start from Cumberland Street N" detail into the notes block instead of dropping it. UCD/DCU tabs not yet converted.
- [x] 2026-09-16 · Added a header background photo (owner-supplied bus photo) behind the red gradient (opacity reduced to 82%/88% so it shows through, verified text stays legible), removed the "SILLAN COACHES"/"Travel in comfort" header text, changed the route line to "Cootehill/Shercock ⇄ Dublin", and removed the white pill background behind the run-header day-range tags in favour of bold white text directly on red.
